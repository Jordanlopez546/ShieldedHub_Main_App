...
const GetStartedAndLoginImageView = ({text}:InputBtnProps) => {
  ...
  const containerStyles = {
    width: width * 0.6, // 60% of the screen
    height: height * 0.3, // 30% of the screen
  };


  return (
    <View style={[styles.imageContainer, containerStyles]}>
      <Image resizeMode='cover' style={styles.image} source={ text === 'first' ? imagePaths.first : imagePaths.second} />
    </View>
  )
}
